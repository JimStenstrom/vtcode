...
use vtcode_config::core::AgentCustomPromptsConfig;
use vtcode_config::constants;

const PROMPTS_SUBDIR: &str = "prompts";

/// Built-in prompts will be embedded at compile time
/// For now, this is a placeholder that will be populated by build.rs
const BUILTIN_PROMPTS: &[(&str, &str)] = &[
    // Placeholder - will be replaced by build.rs
];
        ...
        for (name, contents) in BUILTIN_PROMPTS {
            match CustomPrompt::from_embedded(name, contents) {
                Ok(prompt) => {
                    let key = prompt.name.to_ascii_lowercase();
                    if prompts.contains_key(&key) {
                        continue; // User-defined prompts override built-ins
                    }
                    prompts.insert(key, prompt);
                }
                Err(err) => {
...
fn resolve_directories(config: &AgentCustomPromptsConfig, workspace: &Path) -> Vec<PathBuf> {
    ...
    if let Ok(env_path) = env::var(constants::prompts::CUSTOM_PROMPTS_ENV_VAR) {
...
fn resolve_directory(value: &str, workspace: &Path) -> PathBuf {
    ...
    if trimmed.is_empty() {
        return resolve_directory(constants::prompts::DEFAULT_CUSTOM_PROMPTS_DIR, workspace);
    }
...
mod tests {
    ...
    async fn builtin_prompt_available_without_files() {
        ...
        // For now, we don't have built-in prompts embedded, so registry should be empty
        // This test will be updated once we add the build.rs for embedding prompts
        assert!(!registry.enabled() || registry.is_empty());
    }

    #[test]
    fn builtin_docs_contains_vtcode_docs_map() {
        let builtin_docs = BuiltinDocs::default();
        // For now, built-in docs are empty - will be populated by build.rs
    ...
    fn builtin_prompts_include_embedded_assets() {
        ...
        // For now, built-in prompts are empty - will be populated by build.rs
        // Will update this test once we add the build.rs
        let _names: HashSet<_> = prompts.iter().map(|prompt| prompt.name.as_str()).collect();
    }
}
