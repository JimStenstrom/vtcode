...
impl Session {
    ...
    pub(super) fn header_height_from_lines(&self, width: u16, lines: &[Line<'static>]) -> u16 {
        if width == 0 {
            return self.render_state.header_rows.max(ui::INLINE_HEADER_HEIGHT);
        }
    ...
    pub fn header_block_title(&self) -> Line<'static> {
        ...
        let version = if self.render_state.header_context.version.trim().is_empty() {
            fallback.version
        } else {
            self.render_state.header_context.version.clone()
        };
    ...
    fn header_provider_value(&self) -> String {
        let trimmed = self.render_state.header_context.provider.trim();
        if trimmed.is_empty() {
            InlineHeaderContext::default().provider
        } else {
            self.render_state.header_context.provider.clone()
        }
    }

    fn header_model_value(&self) -> String {
        let trimmed = self.render_state.header_context.model.trim();
        if trimmed.is_empty() {
            InlineHeaderContext::default().model
        } else {
            self.render_state.header_context.model.clone()
        }
    }

    fn header_mode_label(&self) -> String {
        let trimmed = self.render_state.header_context.mode.trim();
        if trimmed.is_empty() {
            InlineHeaderContext::default().mode
        } else {
            self.render_state.header_context.mode.clone()
        }
    }
    ...
    fn header_reasoning_value(&self) -> Option<String> {
        let trimmed = self.render_state.header_context.reasoning.trim();
        let value = if trimmed.is_empty() {
            InlineHeaderContext::default().reasoning
        } else {
            self.render_state.header_context.reasoning.clone()
        };
    ...
    pub fn header_chain_values(&self) -> Vec<String> {
        ...
        let fields = [
            (
                &self.render_state.header_context.workspace_trust,
                defaults.workspace_trust.clone(),
            ),
            (&self.render_state.header_context.tools, defaults.tools.clone()),
            (&self.render_state.header_context.git, defaults.git.clone()),
    ...
    fn header_highlights_line(&self) -> Option<Line<'static>> {
        ...
        for highlight in &self.render_state.header_context.highlights {
    ...
    pub(super) fn header_secondary_style(&self) -> Style {
        let mut style = self.default_style();
        if let Some(secondary) = self.display_state.theme.secondary.or(self.display_state.theme.foreground) {
