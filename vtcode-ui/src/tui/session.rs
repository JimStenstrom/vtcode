...
impl Session {
    ...
    pub fn handle_command(&mut self, command: InlineCommand) {
        match command {
            ...
            InlineCommand::SetPrompt { prefix, style } => {
                self.prompt.prefix = prefix;
                self.prompt.style = style;
                self.ensure_prompt_style_color();
            }
            InlineCommand::SetPlaceholder { hint, style } => {
                self.prompt.placeholder = hint;
                self.prompt.placeholder_style = style;
            }
            ...
            InlineCommand::SetHeaderContext { context } => {
                self.render.header_context = context;
                self.ui.needs_redraw = true;
            }
            InlineCommand::SetInputStatus { left, right } => {
                self.prompt.status_left = left;
                self.prompt.status_right = right;
                self.ui.needs_redraw = true;
            }
            ...
            InlineCommand::SetCursorVisible(value) => {
                self.ui.cursor_visible = value;
            }
            InlineCommand::SetInputEnabled(value) => {
                self.ui.input_enabled = value;
    ...
    pub fn render(&mut self, frame: &mut Frame<'_>) {
        ...
        if self.ui.needs_full_clear {
            ...
            self.ui.needs_full_clear = false;
        }

        // Handle deferred file browser trigger (after slash modal dismisses)
        if self.palette.deferred_file_browser_trigger {
            self.palette.deferred_file_browser_trigger = false;
        ...
        if self.palette.deferred_prompt_browser_trigger {
            self.palette.deferred_prompt_browser_trigger = false;
        ...
        if header_height != self.render.header_rows {
            self.render.header_rows = header_height;
    ...
    fn apply_transcript_rows(&mut self, rows: u16) {
        ...
        if self.ui.transcript_rows != resolved {
            self.ui.transcript_rows = resolved;
            self.invalidate_scroll_metrics();
        }
    }
    ...
    fn apply_transcript_width(&mut self, width: u16) {
        if self.ui.transcript_width != width {
            self.ui.transcript_width = width;
            self.invalidate_scroll_metrics();
        }
    }
    ...
    fn render_transcript(&mut self, frame: &mut Frame<'_>, area: Rect) {
        ...
        self.ui.transcript_view_top = vertical_offset;
    ...
    fn render_file_palette(&mut self, frame: &mut Frame<'_>, viewport: Rect) {
        if !self.palette.file_palette_active {
            return;
        }
    ...
    fn render_prompt_palette(&mut self, frame: &mut Frame<'_>, viewport: Rect) {
        if !self.palette.prompt_palette_active {
            return;
        }
    ...
    fn toggle_timeline_pane(&mut self) {
        self.ui.show_timeline_pane = !self.ui.show_timeline_pane;
        self.invalidate_scroll_metrics();
        self.mark_dirty();
    }
    ...
    fn show_modal(
        ...
        let state = ModalState {
            ...
            restore_input: self.ui.input_enabled,
            restore_cursor: self.ui.cursor_visible,
            search: None,
        };
        if state.secure_prompt.is_none() {
            self.ui.input_enabled = false;
        }
        self.ui.cursor_visible = false;
        self.render.modal = Some(state);
        self.mark_dirty();
    }
    ...
    fn close_modal(&mut self) {
        if let Some(state) = self.render.modal.take() {
            self.ui.input_enabled = state.restore_input;
            self.ui.cursor_visible = state.restore_cursor;
            // Force full screen clear on next render to remove modal artifacts
            self.ui.needs_full_clear = true;
            // Force transcript cache invalidation to ensure full redraw
            self.render.transcript_cache = None;
            self.mark_dirty();
        }
    }
    ...
    fn render_modal(&mut self, frame: &mut Frame<'_>, viewport: Rect) {
        ...
        let Some(modal) = self.render.modal.as_mut() else {
            return;
        };
    ...
    pub fn set_custom_prompts(&mut self, custom_prompts: CustomPromptRegistry) {
        ...
        if custom_prompts.enabled() && !custom_prompts.is_empty() {
            ...
            self.palette.prompt_palette = Some(palette);
        }

        self.palette.custom_prompts = Some(custom_prompts);
    ...
    fn load_file_palette(&mut self, files: Vec<String>, workspace: std::path::PathBuf) {
        ...
        self.palette.file_palette = Some(palette);
        self.palette.file_palette_active = false;
        self.check_file_reference_trigger();
    }

    fn check_file_reference_trigger(&mut self) {
        if let Some(palette) = self.palette.file_palette.as_mut() {
                ...
                extract_file_reference(self.input_manager.content(), self.input_manager.cursor())
            {
                ...
                self.palette.file_palette_active = true;
            } else {
                self.palette.file_palette_active = false;
            }
        }
    ...
    fn check_prompt_reference_trigger(&mut self) {
        ...
        if let Some(palette) = self.palette.prompt_palette.as_mut() {
            if let Some((_, _, query)) =
                extract_prompt_reference(self.input_manager.content(), self.input_manager.cursor())
            {
                ...
                palette.reset();
                palette.set_filter(query);
                self.palette.prompt_palette_active = true;
            } else {
                self.palette.prompt_palette_active = false;
            }
        }
    }
    ...
    fn handle_prompt_palette_key(&mut self, key: &KeyEvent) -> bool {
        ...
        let Some(palette) = self.palette.prompt_palette.as_mut() else {
            return false;
        };
    ...
    fn process_key(&mut self, key: KeyEvent) -> Option<InlineEvent> {
        ...
        if let Some(modal) = self.render.modal.as_mut() {
        ...
        match key.code {
            ...
            KeyCode::Up => {
                let history_requested = if self.ui.input_enabled && (has_alt || has_command) {
                    true
                } else if self.ui.input_enabled {
    ...
    fn append_text(&mut self, kind: InlineMessageKind, text: &str, style: &InlineTextStyle) {
        ...
        {
            if let Some(line) = self.display.lines.last_mut() {
                if line.kind == kind {
    ...
    fn ensure_reflow_cache(&mut self, width: u16) -> &mut TranscriptReflowCache {
        ...
        if first_dirty == self.display.lines.len() {
            ...
            self.render.transcript_cache = Some(cache);
    ...
    fn handle_tool_code_fence_marker(&mut self, text: &str) -> bool {
        ...
        if self.display.in_tool_code_fence {
            self.display.in_tool_code_fence = false;
            self.remove_trailing_empty_tool_line();
        } else {
            self.display.in_tool_code_fence = true;
        }
