...
**For practical examples and detailed use cases, see [Lifecycle Hooks Examples](./lifecycle-hooks-examples.md).**
...
* Make scripts executable with `chmod +x` to avoid permission errors.
* Use `jq` for JSON parsing in shell scripts for robust payload handling.
* Test hooks manually with sample payloads before using in production.

## Quick Start

1. **Create `vtcode.toml`** in your project root with a `[hooks.lifecycle]` section
2. **Create hooks directory**: `mkdir -p .vtcode/hooks`
3. **Write hook scripts** and make them executable: `chmod +x .vtcode/hooks/*.sh`
4. **Configure hooks** in `vtcode.toml` referencing your scripts
5. **Validate**: `vtcode config validate`
6. **Test manually** with sample JSON payloads

## Next Steps

* **[Lifecycle Hooks Examples](./lifecycle-hooks-examples.md)** - Detailed examples and use cases
* **[Security Guide](./security.md)** - Security considerations for hooks
* **[Configuration Reference](../config.md)** - Complete configuration options

## See Also

* Claude Code Hooks documentation: https://docs.claude.com/en/docs/claude-code/hooks
* VTCode configuration guide
