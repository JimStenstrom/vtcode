...
| 4 | **Sub-agents** | `vtcode-core/src/subagents/mod.rs` | `SubagentController` spawns child agents with `spawn_with_spec()`. When `isolation == "worktree"`, the child runs in a git worktree. After the child finishes, `run_worktree_reconciliation()` runs a verify-then-merge cycle. |
